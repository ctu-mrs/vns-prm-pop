...
MeshObject::MeshObject(std::string objectname) :
...
void MeshObject::dropOutOfXY(double xMin, double xMax, double yMin, double yMax) {

	for (int var = 0; var < numVertices; ++var) {
		Point3D actualPoint = vertices[var];
		if (actualPoint.x < xMin || actualPoint.x > xMax || actualPoint.y < yMin || actualPoint.y > yMax) {
...
void MeshObject::addFace(Face face) {
	addFace(face.id1, face.id2, face.id3);
}
...
int MeshObject::createRAPIDModel() {
	...
	for (int var = 0; var < numFaces; ++var) {
		//INFO("face "<<var<<" with vertices "<<faces[var].id1<<" "<<faces[var].id2<<" "<<faces[var].id3)
...
Point3D* MeshObject::getVertices() {
	return vertices;
}
Face* MeshObject::getFaces() {
	return faces;
}

#ifdef COLOR_ARRAY
Color* MeshObject::getColors() {
	return colors;
}
#endif

Vector3D* MeshObject::getNormals() {
	return normals;
}
unsigned int MeshObject::getVerticesNum() {
	return numVertices;
}
unsigned int MeshObject::getFacesNum() {
	return numFaces;
}
unsigned int MeshObject::getNormalsNum() {
	return numNormals;
}
Position3D MeshObject::getPosition() {
	return this->position;
}
Position3D* MeshObject::getPositionPntr() {
	return &(this->position);
}
void MeshObject::setPosition(Position3D pos) {
	this->position = pos;
}
RAPID_model* MeshObject::getRapidModel() {
	return rapidModel;
}
...
void MeshObject::printVerteces() {
	Point3D objectMean(this->position.getX(), this->position.getY(), this->position.getZ());
...
std::string MeshObject::getName() {
	return this->name;
}

bool MeshObject::collide(MeshObject* object1, Position3D object1Position, MeshObject* object2,
		Position3D object2Position) {
	//INFO("collide");
	...
	RAPID_Collide(object1RM.M, (double*) &object1TV, object1->getRapidModel(), object2RM.M, (double*) &object2TV,
			object2->getRapidModel(), RAPID_FIRST_CONTACT);
...
bool MeshObject::collide(MeshObject* object1, MeshObject* object2) {
	return MeshObject::collide(object1, object1->getPosition(), object2, object2->getPosition());
}

bool MeshObject::collide(std::vector<MeshObject*> * obstacles, MeshObject* robot, Position3D robotPosition) {
	...
	for (int var = 0; var < obstacles->size(); ++var) {
		...
		RAPID_Collide(robotRotation.M, (double*) &robotTranslation, robot->getRapidModel(), obstacleRotation.M,
				(double*) &obstacleTranslation, (*obstacles)[var]->getRapidModel(), RAPID_FIRST_CONTACT);
...
std::string MeshObject::getMaterialName() {
