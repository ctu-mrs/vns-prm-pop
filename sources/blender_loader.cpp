	...
	if (!in) {
		...
		while (getline(in, line)) {
			...
			if (line.substr(0, 2) == "o ") {
				...
				INFO(
						"found material " <<materialName<< " for object "<<loadedObjects.back()->getName());
		...
		for (int var = 0; var < loadedObjects.size(); ++var) {
			INFO(
					"object "<<var<<" "<<loadedObjects[var]->getName()<<" has faces "<<loadedObjects[var]->getFacesNum())
			/*
	...
	if (!in) {
		...
		while (getline(in, line)) {
			if (line.substr(0, 7) == "newmtl ") {
				...
				std::string materialName;
				s >> materialName;
				INFO("fond material "<<materialName);
				...
				INFO("fond color "<< c << " for material "<<actualMaterial);
			} else { /* ignoring this line */
			}
		}
		...
		for (objectIndex = 0; objectIndex < meshObjects.size(); ++objectIndex) {
			for (materialIndex = 0; materialIndex < loadedMaterialNames.size();
				...
				if (meshObjects[objectIndex]->getMaterialName()
					...
					INFO(
							"set color "<< color << " to object "<<meshObjects[objectIndex]->getName());
				}
			}
		}
	}
...
int BlenderLoader::saveObjectsToFile(const char* filename,
		std::vector<MeshObject*> objects, bool addObjectPosition) {
	...
	for (int objectIndex = 0; objectIndex < objects.size(); ++objectIndex) {
		...
		for (int verticeIndex = 0;
			...
			if (actual_vertex_id % pritAfter == 0) {
				INFO(
						"saved vertices " << actual_vertex_id <<"/" << objects[objectIndex]->getVerticesNum());
			}
		}
		INFO(
				"saved vertices " << actual_vertex_id <<"/" << objects[objectIndex]->getVerticesNum());
		for (int faceIndex = 0; faceIndex < objects[objectIndex]->getFacesNum();
				++faceIndex) {
			myfile << "f "
					<< (objects[objectIndex]->getFaces()[faceIndex].id1
							+ substract_id) << " "
					<< (objects[objectIndex]->getFaces()[faceIndex].id2
							+ substract_id) << " "
					<< (objects[objectIndex]->getFaces()[faceIndex].id3
							+ substract_id) << std::endl;
		}
	}
	myfile.close();
	return NO_ERROR;
