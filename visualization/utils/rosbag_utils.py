...
def loadOdometryWhole(bag, uav_name, odom_topic="/mbzirc_odom/new_odom"):
...
def loadOdometryInTracking(bag, uav_name, odom_topic="/mbzirc_odom/new_odom"):
...
def loadOdometryAfterArming(bag, uav_name, odom_topic="/mbzirc_odom/new_odom"):
...
def loadOdometryAfterArmingWithRTKz(bag, uav_name, odom_topic="/mbzirc_odom/new_odom"):
...
def loadOdometryAfterArmingEntTime(bag, uav_name, end_time_relative, odom_topic="/mbzirc_odom/new_odom"):
...
def loadOdometryBetweenArming(bag, uav_name):
    ...
    odom_topic = "/" + uav_name + "/mbzirc_odom/new_odom"
    mavros_state_topic = "/" + uav_name + "/mavros/state"
    topics_to_read = [odom_topic, mavros_state_topic]   
    print(topics_to_read)
    for topic in topics_to_read:
    ...
    for topic, msg, t in bag.read_messages():
        ...
        if topic == full_topic_name :
            ...
            if (not between_time or (between_time and topic_time >= start_time and topic_time <= end_time)):
                ...
                image_name = os.path.join(str(save_dir) , topic_parts[1] + "-" + format(image_index, index_format) + image_type)
                if time_image_name:
                    image_name =  os.path.join( str(save_dir) , topic_parts[1] + "-" + str(t) + "s" + image_type)
                cv2.imwrite(image_name, cv_image)
                image_index = image_index + 1

def get_info(bag, compressed_topic, topic=None, start_time=rospy.Time(0), end_time=rospy.Time(sys.maxint)):
    size = (0, 0)
    times = []
    bridge = CvBridge()
    ...
    iterator = bag.read_messages(topics=topic, start_time=start_time, end_time=end_time)  # , raw=True)
...
def write_frames(bag, writer, total, compressed_topic, topic=None, nframes=repeat(1), start_time=rospy.Time(0), end_time=rospy.Time(sys.maxint), viz=False, encoding='bgr8'):
    bridge = CvBridge()
    count = 1
...
def savevideo(bag, uav_name, image_topic, save_dir,between_time=False, start_time=rospy.Time(0), end_time=rospy.Time(sys.maxint),precision = 1):            
    print('Calculating video properties')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    if not between_time:
        start_time=rospy.Time(0)
        end_time=rospy.Time(sys.maxint)
    compressed_topic = False
    if 'compressed' in image_topic:
        compressed_topic = True
    full_topic_name = '/' + uav_name + image_topic
    rate, minrate, maxrate, size, times = get_info(bag, compressed_topic, full_topic_name,start_time=start_time, end_time=end_time)
    encoding = 'bgr8'
    nframes = calc_n_frames(times, precision)
   
    # writer = cv2.VideoWriter(outfile, cv2.cv.CV_FOURCC(*'DIVX'), rate, size)
    writer = cv2.VideoWriter(os.path.join(save_dir,"video.avi"), cv2.VideoWriter_fourcc(*'XVID'), np.ceil(maxrate * precision), size)
    print('Writing video to',os.path.join(save_dir,"video.avi"))
    
    write_frames(bag, writer, len(times), compressed_topic, topic=full_topic_name, nframes=nframes, encoding=encoding,start_time=start_time, end_time=end_time)
    writer.release()
    print('\n')
